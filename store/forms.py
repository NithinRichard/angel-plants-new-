...
class TrackOrderForm(forms.Form):
    """Form for tracking orders by order number and email."""
    order_number = forms.CharField(
        ...
        widget=forms.TextInput(attrs={
            'class': 'form-control',
            'placeholder': _('Enter your order number'),
            'autocomplete': 'off',
            'autocapitalize': 'off',
            'spellcheck': 'false'
        })
    ...
    email = forms.EmailField(
        ...
        widget=forms.EmailInput(attrs={
            'class': 'form-control',
            'placeholder': _('Enter the email used for this order'),
            'autocomplete': 'email'
        })
    )

    def clean_order_number(self):
        order_number = self.cleaned_data.get('order_number', '').strip().upper()
        if not order_number:
            raise forms.ValidationError(_('Please enter your order number.'))
        return order_number
